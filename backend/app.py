...
def api_alertas():
    ...
    try:
        ...
        def _normalizar_bool(valor):
            ...
            if texto in ("true", "sim", "s", "1", "yes", "y"):
                return True

            if texto in ("false", "nao", "não", "n", "0", "no"):
                return False
        ...
        def _calcular_itens_faltando_checklist(registro):
            ...
            itens_marcados = (
                detalhe.get("itens_marcados")
                or detalhe.get("itensMarcados")
                or detalhe.get("itens")
                or registro.get("checklistEntrada")
                or []
            )
        ...
        def _veiculo_danificado_entrada_alerta(registro):
            ...
            veiculo_danificado = _normalizar_bool(
                ...
                or detalhe.get("possui_dano")
                or detalhe.get("com_dano")
            )

            estado_veiculo = _normalizar_texto(
                detalhe.get("estado_veiculo")
                or detalhe.get("condicao_veiculo")
                or detalhe.get("situacao_veiculo")
                or ""
            )
        ...
        def _veiculo_danificado_saida_alerta(registro):
            ...
            valor = _normalizar_bool(
                registro.get("veiculoDanificadoSaida")
                or registro.get("veiculo_danificado_saida")
                or registro.get("danificado_saida")
                or registro.get("possuiDanoSaida")
            )
